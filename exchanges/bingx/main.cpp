    ...
    try {
        ...
        ws.set_option(websocket::stream_base::decorator(
            [](websocket::request_type& req) {
                req.set(beast::http::field::user_agent, "bingx-connector");
            }
        ));

        ws.handshake("open-api-ws.bingx.com", "/market");

        std::string subscribe_msg = R"({"id":"1","dataType":"BTC-USDT@kline_1m"})";
        ws.write(net::buffer(subscribe_msg));

        std::cout << "Connected to BingX WebSocket (BTC-USDT 1m kline)" << std::endl;
