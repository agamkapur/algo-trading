    ...
    try {
        ...
        ws.set_option(websocket::stream_base::decorator(
            [](websocket::request_type& req) {
                req.set(beast::http::field::user_agent, "coinbase-connector");
            }
        ));

        ws.handshake("ws-feed.exchange.coinbase.com", "/");

        std::string subscribe_msg = R"({"type":"subscribe","product_ids":["BTC-USDT"],"channels":["ticker"]})";
        ws.write(net::buffer(subscribe_msg));

        std::cout << "Connected to Coinbase WebSocket (BTC-USDT ticker)" << std::endl;
