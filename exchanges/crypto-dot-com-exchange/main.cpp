    ...
    try {
        ...
        ws.set_option(websocket::stream_base::decorator(
            [](websocket::request_type& req) {
                req.set(beast::http::field::user_agent, "crypto-com-connector");
            }
        ));

        ws.handshake("stream.crypto.com", "/v2/market");

        std::string subscribe_msg = R"({"id":1,"method":"subscribe","params":{"channels":["candlestick.1m.BTC_USDT"]},"nonce":1})";
        ws.write(net::buffer(subscribe_msg));

        std::cout << "Connected to Crypto.com WebSocket (BTC_USDT 1m candlestick)" << std::endl;
