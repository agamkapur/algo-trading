    ...
    try {
        ...
        ws.set_option(websocket::stream_base::decorator(
            [](websocket::request_type& req) {
                req.set(beast::http::field::user_agent, "bitfinex-connector");
            }
        ));

        ws.handshake("api-pub.bitfinex.com", "/ws/2");

        std::string subscribe_msg = R"({"event":"subscribe","channel":"candles","key":"trade:1m:tBTCUSD"})";
        ws.write(net::buffer(subscribe_msg));

        std::cout << "Connected to Bitfinex WebSocket (tBTCUSD 1m candles)" << std::endl;
